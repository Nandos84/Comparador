...
/////////////// BLOCO ADICIONADO
  if (this.isViewerEmbedded && window.parent && rawQuery) {
    // Verifica se a busca foi concluída (não está mais em PENDING)
    if (state === 1 || state === 2) { // 1 = FOUND, 2 = NOT_FOUND
      window.parent.postMessage({
        type: 'pdfSearchResults',
        matchesCount: matchesCount.total || 0,
        activeMatchIndex: (matchesCount.current || 1) - 1,
        query: rawQuery,
        state: state,
        found: state === 1 // true se encontrou, false se não encontrou
      }, '*');
      
      console.log('PDF enviando resultado:', {
        query: rawQuery,
        found: state === 1,
